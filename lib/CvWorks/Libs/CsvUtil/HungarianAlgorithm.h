#ifndef _HUNGARIANALGORITHM_H
#define _HUNGARIANALGORITHM_H
...
/*********************************************************************************************************/
/* Funções para resolver o 'assignment problem'. 
Copiadas da biblioteca: FDDB face evaluation. Originalmente criada por Brian Gerkey, 2003 */


static double** array_to_matrix(double* m, int rows, int cols) {
	/// Garante que cols>=rows
	int colsToAdd=0;
	if(cols<rows)
		colsToAdd=rows-cols;
  ...
  for(i=0;i<rows;i++)
  {
	r[i] = (double*)calloc((cols+colsToAdd),sizeof(double));
	for(j=0;j<cols;j++)
	  r[i][j] = m[i*cols+j];
 ...
 */
static void hungarian_init(hungarian_t* prob, double** r, size_t m, size_t n, int mode)
{
  assert(prob);
  ...
  (prob->u = (double*)calloc(m,sizeof(double)));
  (prob->v = (double*)calloc(n,sizeof(double)));
  (prob->seq.i = (unsigned int*)calloc(m*n,sizeof(unsigned int)));
  (prob->seq.j = (unsigned int*)calloc(m*n,sizeof(unsigned int)));
 ...
 */
static void hungarian_fini(hungarian_t* prob)
{
  assert(prob);
 ...
 */
static void hungarian_make_cover(hungarian_t* prob)
{
  double* row_max; 
  double* col_max; 
...
static void hungarian_build_q(hungarian_t* prob)
{
  assert(prob);
...
static void hungarian_add_stars(hungarian_t* prob)
{
  unsigned int i,j,k;
 ...
 * Kuhn's Routine I
 */
static hungarian_code_t hungarian_routine_one(hungarian_t* prob)
{
  assert(prob);
  prob->seq.k=0;
  for(unsigned int i=0;i<prob->m;i++)
 ...
 * Kuhn's Routine II
 */
static hungarian_code_t hungarian_routine_two(hungarian_t* prob)
{
  assert(prob);
 ...
 */
static void hungarian_solve(hungarian_t* prob)
{
 ...
 * debugging.
 */
static void hungarian_print_stars(hungarian_t* prob)
{
  double benefit=0;
  assert(prob);
  puts("\nQ: ");
 ...
 */
static void hungarian_print_assignment(hungarian_t* prob)
{
  assert(prob);
  puts("\nA:");
  for(unsigned int i=0;i<prob->m;i++)
 ...
 */
static void hungarian_print_rating(hungarian_t* prob)
{
  puts("\nR: ");
  for(unsigned int i=0;i<prob->m;i++)
  {
 ...
 */
static int hungarian_check_feasibility(hungarian_t* prob)
{
  char assigned;
 ...
 * called hungarian_solve() first.
 */
static double hungarian_benefit(hungarian_t* prob)
{
  double benefit=0;
  assert(prob);
  for(unsigned int i=0;i<prob->m;i++)
...
#endif // ndef _HUNGARIANALGORITHM_H
